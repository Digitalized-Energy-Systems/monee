...
def build_ppipes_line(spec, fluid, t_k, thermal=False):
    ...
    j = [
        ppi.create_junction(net, pn_bar=P_REF_BAR, tfluid_k=t_k)
...
def run_gas_suite():
    ...
    for name, spec in cases:
        ...
        print(
            f"  {name:18s} pp {t_pp * 1000:6.2f}ms  monee {t_mn * 1000:7.1f}ms  "
...
def run_heat_suite():
    ...
    for name, spec in cases:
        ...
        rows.append(
            _row("HEAT", name, spec["n"], p_pp, p_mn, t_pp, t_mn, temp_pp, temp_mn)
        )
        print(
            f"  {name:18s} pp {t_pp * 1000:6.2f}ms  monee {t_mn * 1000:7.1f}ms  "
...
class CHPControlMultiEnergy(G2PControlMultiEnergy):
    ...
    def control_step(self, multinet):
        gas = float(
            multinet["nets"][self.name_net_gas].sink.at[
        ...
        multinet["nets"][self.name_net_heat].heat_exchanger.at[
            self.idx_heat, "qext_w"
        ] = -gas * conv * self.eff_heat * 1e6
        self.applied = True
...
def build_ppipes_mes(p_el_mw):
    ...
    ppi.create_pipe_from_parameters(
        ...
        from_junction=j0,
        to_junction=j1,
        length_km=_MES_GAS_L / 1000.0,
        diameter_m=_MES_GAS_D,
        k_mm=K_MM,
    )
    ...
    P2GControlMultiEnergy(
        mnet, lid, src, efficiency=_MES_EFF, name_power_net="power", name_gas_net="gas"
    )
...
def build_monee_mes(g_produced_kgs):
    ...
    gas = mm.create_gas_grid("gas", type="lgas")
    gas.pressure_ambient_pa = (
        STANDARD_ATMOSPHERE_PA  # gauge pressures (match pandapipes)
    )
    b0 = net.node(
        mm.Bus(base_kv=20),
        mm.EL,
    ...
    net.branch(
        mm.PowerLine(length_m=2000, r_ohm_per_m=0.0003, x_ohm_per_m=0.0002, parallel=1),
        b0,
        b1,
    )
    ...
    gj1 = net.node(
        mm.Junction(),
        child_ids=[net.child(mm.Sink(mass_flow_kgs=_MES_SINK_KGS))],
        grid=gas,
    )
    net.branch(
        mm.GasPipe(diameter_m=_MES_GAS_D, length_m=_MES_GAS_L, roughness_m=ROUGHNESS_M),
        gj0,
        gj1,
    )
    mx.create_p2g(
        ...
        to_node_id=gj1,
        efficiency=_MES_EFF,
        mass_flow_setpoint_kgs=g_produced_kgs,
        regulation=1,
    )
...
def build_ppipes_mes_g2p(g2p_gas_kgs):
    ...
    ppi.create_sink(
        gn, junction=j1, mdot_kg_per_s=_G2P_BASE_SINK_KGS
    )  # base gas demand
    g2p_sink = ppi.create_sink(
        gn, junction=j1, mdot_kg_per_s=g2p_gas_kgs
    )  # G2P consumption
    mnet = create_empty_multinet("mes")
    add_net_to_multinet(mnet, pn, "power")
    add_net_to_multinet(mnet, gn, "gas")
...
def build_monee_mes_g2p(p_produced_mw):
    ...
    gj1 = net.node(
        mm.Junction(),
        child_ids=[net.child(mm.Sink(mass_flow_kgs=_G2P_BASE_SINK_KGS))],
        grid=gas,
    )
    net.branch(
        mm.GasPipe(diameter_m=_MES_GAS_D, length_m=_MES_GAS_L, roughness_m=ROUGHNESS_M),
...
def build_ppipes_mes_chp(gas_kgs, water_flow_kgs, t_in_k):
    ...
    ch = ppi.create_heat_exchanger(
        hn, h1, h2, qext_w=-1.0, inner_diameter_mm=150.0
    )  # CHP heat output
    ...
    CHPControlMultiEnergy(
        mnet, sg, cg, ch, _CHP_EFF_P, _CHP_EFF_H, element_type_power="sgen"
    )
...
def build_monee_mes_chp(gas_kgs):
    ...
    gj1 = net.node(
        mm.Junction(), grid=gas, child_ids=[net.child(mm.Sink(mass_flow_kgs=0.0))]
    )
    net.branch(
        mm.GasPipe(diameter_m=_MES_GAS_D, length_m=_MES_GAS_L, roughness_m=ROUGHNESS_M),
        gj0,
    ...
    w_ret = net.node(
        mm.Junction(),
        grid=mm.WATER_KEY,
        child_ids=[net.child(mm.ExtHydrGrid(t_k=_CHP_T_IN))],
    )
    w_sup = net.node(mm.Junction(), grid=mm.WATER_KEY)
    w_c = net.node(
        mm.Junction(),
        grid=mm.WATER_KEY,
        child_ids=[net.child(mm.ConsumeHydrGrid(_CHP_MDOT_W))],
    )
    ...
    mx.create_chp(
        ...
        power_node_id=b1,
        heat_node_id=w_ret,
        heat_return_node_id=w_sup,
        gas_node_id=gj1,
        mass_flow_setpoint_kgs=gas_kgs,
        diameter_m=0.15,
        efficiency_power=_CHP_EFF_P,
        efficiency_heat=_CHP_EFF_H,
        regulation=1,
    )
    net.apply_formulation(EL_NLP_FORMULATION)
    net.apply_formulation(make_gas_nlp_formulation(friction_model="hybrid"))
...
def run_mes_suite():
    ...
    for name, p_el in [
        ...
        rmn, t_mn = _time(
            lambda: run_energy_flow(build_monee_mes(g_produced)), repeats=1
        )
        vm_mn = rmn.dataframes["Bus"]["vm_pu"].to_numpy(float)
        p_mn = rmn.dataframes["Junction"]["pressure_pu"].to_numpy(float) * P_REF_BAR

        rows.append(
            _row("MES", name, 2, p_pp, p_mn, t_pp, t_mn, vm_pp=vm_pp, vm_mn=vm_mn)
        )
        print(
            f"  {name:16s} pp {t_pp * 1000:6.2f}ms  monee {t_mn * 1000:7.1f}ms  "
    ...
    for name, g2p_gas in [
        ...
        rmn, t_mn = _time(
            lambda: run_energy_flow(build_monee_mes_g2p(p_produced)), repeats=1
        )
        vm_mn = rmn.dataframes["Bus"]["vm_pu"].to_numpy(float)
        p_mn = rmn.dataframes["Junction"]["pressure_pu"].to_numpy(float) * P_REF_BAR
    ...
    for name, gas_kgs in [("mes CHP 0.10kgs", 0.10)]:
        ...
        row["t_err_k"] = abs(
            dT_m - dT_p
        )  # CHP heat-exchanger temperature-rise agreement (K)
        rows.append(row)
        print(
            f"  {name:16s} pp {t_pp * 1000:6.2f}ms  monee {t_mn * 1000:7.1f}ms  "
...
def main():
    ...
    with _silent():
        try:
            ppi.pipeflow(
                build_ppipes_line(line_spec(3, 0.3, 1000, 0.2), "lgas", GAS_T_K)
            )
    ...
    if not mes.empty:
        print(
            f"coupled el+gas: bus voltage agrees to {mes.vm_err_pu.max():.2e} pu, "
