...
def test_sanitize_apm_name():
    # GIVEN
    simple_name = "powergenerator-5.p_mw"
    branch_name = "genericpowerbranch-(0, 1, 0).p_from_mw"

    # WHEN
    sanitized_simple = sanitize_apm_name(simple_name)
    sanitized_branch = sanitize_apm_name(branch_name)

    # THEN
    assert sanitized_simple == "powergenerator_5_p_mw"
    assert sanitized_branch == "genericpowerbranch_0_1_0_p_from_mw"


def test_parse_infeasibilities():
    # GIVEN
    output = _SAMPLE_INFEASIBILITIES

    # WHEN
    equations = parse_infeasibilities(output)
...
def test_parse_infeasibilities_with_name_map():
    ...
    # WHEN
    equations = parse_infeasibilities(output, name_map=_NAME_MAP)
...
def test_infeasible_monee_gekko_solve():
    ...
    # Infeasible: ext-grid must cover the 2 MW load but is constrained to [0.1, 0.5] MW.
    ...
    # THEN
    # The raised error carries the diagnostic report instead of the bare "Solution Not Found".
