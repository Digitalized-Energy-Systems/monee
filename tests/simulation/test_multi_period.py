...
def test_single_period_matches_single_step():
    ...
    # THEN
    assert r_single.success
    assert r_multi.success

    # multi-period with T=1 must match the single-step solver
    vm_single = r_single.get(mm.Bus)["vm_pu"].sort_index()
    vm_multi = r_multi.get_result_for(mm.Bus, "vm_pu")
...
def test_storage_soc_initial_condition_pinned():
    ...
    # initial-condition constraint: e_mwh[0] = e_initial + dt_h * p_mw[0]
...
def test_storage_soc_continuity():
    ...
    # e_mwh[t] = e_mwh[t-1] + dt_h * p_mw[t] across all adjacent periods
    soc_df = result.get_result_for(ElectricStorage, "e_mwh")
    p_df = result.get_result_for(ElectricStorage, "p_mw")
    for t in range(1, 4):
...
def test_get_result_for_id():
    ...
    # Series with one entry per period
...
def test_getitem_component():
    ...
    # DataFrame with rows=periods, cols=attributes
    df = result[bat_id]
...
def test_initial_state_override():
    ...
    # different initial conditions must yield different trajectories
    ...
    # override's t=0 constraint is anchored to 2.0
...
def test_terminal_state_constraint():
    ...
    # terminal_state pins e_mwh at t=T-1 to the target
...
def test_mpc_state_propagation():
    ...
    # SoC continuity must hold at all period (incl. window) boundaries
    soc = result.get_result_for(ElectricStorage, "e_mwh")[bat_id]
...
def test_repr_shows_temporal_evolution():
    ...
    # with varying loads the SoC changes, so the section must appear
...
def test_when_period_constraint():
    ...
    # at period 2 the constraint forces p_mw ≈ 1.0; other periods are free
...
def test_when_period_single_period():
    ...
    prob = _storage_problem()
    cons = Constraints()
    # filter targets period 5 but only t=0 exists, so the constraint must not fire
    cons.select(lambda c: isinstance(c.model, ElectricStorage)).equation(
...
def test_controllable_to_attr_no_leak():
    ...
    # only the last period's _apply entries should remain, not 4x accumulation
...
def test_objective_data_via_timeseries():
    ...
    # different price schedules yield different total costs
...
def test_temporal_equation_ramp_rate():
    ...
    # THEN
    assert r_free.success
    assert r_ramp.success

    ext_free = r_free.get_result_for(mm.ExtPowerGrid, "p_mw")[ext_id]
    ext_ramp = r_ramp.get_result_for(mm.ExtPowerGrid, "p_mw")[ext_id]
    max_delta_free = _max_abs_period_delta(ext_free)
    max_delta_ramp = _max_abs_period_delta(ext_ramp)
...
def test_temporal_equation_with_when_period():
    ...
    # period 0→1 should be ramp-limited (≤ 1.0); other periods are free
...
def test_multi_period_load_shedding():
    ...
    # GIVEN
    net, b0, b1, load_id = _simple_power_net()
    # periods 1 and 3 exceed the ext-grid capacity of 3 MW
    td = TimeseriesData()
    ...
    # regulation ~1.0 where load ≤ 3 MW, < 1.0 where load must be shed
...
def test_multi_period_load_shedding_ramp():
    ...
    reg_free = r_free.get_result_for(mm.PowerLoad, "regulation")[load_id]
    reg_ramp = r_ramp.get_result_for(mm.PowerLoad, "regulation")[load_id]
    max_delta_free = _max_abs_period_delta(reg_free)
    max_delta_ramp = _max_abs_period_delta(reg_ramp)
