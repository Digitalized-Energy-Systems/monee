import math
import numpy as np
import monee.model.phys.quadratic_convex.cq as opfmodel
...
class QCElectricityNodeFormulation(NodeFormulation):
    def equations(
        ...
        **kwargs,
        ...
        return opfmodel.square_relax( #auxiliary variable for v²
...
class QCElectricityBranchFormulation(BranchFormulation):
    def ensure_var(self, branch):
        ...
        # angle difference across branch
        branch.va_diff = Var(0, min=-theta_u, max=theta_u)

        # relaxed cosine and sine variable
        branch.cs = Var(1, min=math.cos(theta_u), max=1)
        branch.s = Var(0, min=-1, max=1)

        # auxiliary variables
        branch.vv = Var(1, min=0) #equ 17
        branch.wc = Var(1) #cosine product term, equ 18
        branch.ws = Var(0) #sine product term , equ 19

        # current variable, in sec.4.2 , I in paper (maybe also l?)
    ...
    def equations(self, branch, grid, from_node_model, to_node_model, **kwargs):
        ...
        # QC Power flow equations (Sec 4.1 equations)
        eqs += [opfmodel.int_flow_from_p(
        ...
        # Relaxation strengthening sec 4.2
        eqs += [
            opfmodel.current_flow_equation( #equ 23
                i_var=branch.i_qc,
                v_sq_from_var=from_node_model.vars["vm_pu_squared"],
                v_sq_to_var=to_node_model.vars["vm_pu_squared"],
                wc_var=branch.wc,
            ...
            opfmodel.current_soc_relax( #equ 21
                p_var=branch.p_from_mw,
                q_var=branch.q_from_mvar,
                i_var=branch.i_qc,
                v_sq_ub=from_node_model.vars["vm_pu_squared"],
                #v_sq_ub=v_from_max**2, #not as exact?
            ),
        ]

        # current equations same as in AC formulation, not in paper though?
        eqs += [
            branch.i_from_ka
