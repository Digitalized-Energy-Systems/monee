...
where ``drop_term`` is ``friction · m · |m|`` for the analytic friction models and
a single odd cubic spline ``ψ(m)`` for the PWL model.
"""
...
def smooth_abs(signed, eps, sqrt_impl=math.sqrt):
    """Smooth absolute value √(m² + ε²). ε is a small mass-flow scale [kg/s]."""
...
def darcy_pressure(p_i, p_j, drop_term, length_m, diameter_m, fluid_density):
    """Signed Darcy-Weisbach: ``(p_i-p_j) == -K · drop_term`` with
    ``K = L / (2·ρ·A²·D)``."""
    area = hyd.calc_pipe_area(diameter_m)
    K = length_m / (2.0 * fluid_density * area**2 * diameter_m)
...
def smooth_friction_blend(
    ...
    f_lam = 64.0 / (Re + 1.0)
    f_turb = hyd.swamee_jain(Re, diameter_m, roughness, log_impl)
...
def signed_psi_breakpoints(
    ...
    def psi(m):
        Re = m * diameter_m / (dynamic_visc * area)
        return hyd.friction_value(Re, diameter_m, roughness) * m * m
...
def drop_term_and_eqs(
    friction_model, model, dynamic_visc, area, signed, mag, m_max, **kwargs
):
    """Return ``(drop_term, eqs)`` for the pressure-drop equation.

    ``constant``/``nonlinear`` → ``friction · m · |m|`` (plus the Reynolds/blend
    closure for ``nonlinear``); ``pwl`` → an odd cubic spline ``ψ(signed)``.
    """
    if friction_model == "pwl":
        xs, ys = signed_psi_breakpoints(
            model.diameter_m, model.roughness, dynamic_visc, area, m_max
        )
    ...
    return drop_term, [
        hyd.reynolds_equation(
            model.reynolds, mag, model.diameter_m, dynamic_visc, area
        ),
        model.friction
