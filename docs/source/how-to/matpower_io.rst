...
using a few conventions worth knowing before you optimise:
...
.. list-table::
   ...
   * - ``gen`` rows
     - An :class:`~monee.model.child.ExtPowerGrid` (slack) when the heuristic
       ``Pg == 0`` **and** ``Pg != Pmax`` matches - in that case ``p_mw`` and
       ``q_mvar`` become free variables and ``vm_pu`` is pinned to the
       generator voltage setpoint. All other rows become fixed
       :class:`~monee.model.child.PowerGenerator` children.
   * - ``branch`` rows
...
If you need the serialized form in memory - for custom storage back-ends,
message passing, or diffing - use the dict-level pair
...
The current on-disk structure is **version 2**
(:data:`~monee.io.native.FORMAT_VERSION`); the version is bumped whenever the
structure changes in a non-additive way. Each model attribute is encoded with
full fidelity:
...
Deserialization resolves model classes **by name** through the registry
populated by the :func:`~monee.model.model` decorator. Custom model classes
therefore round-trip only when decorated:
...
:class:`~monee.io.native.PersistenceException` with exactly this hint
("Maybe you forgot to decorate your model class with @model?"). Make sure the
module defining your custom models is imported before calling
