...
:class:`~monee.model.core.MultiGridCompoundModel`. Like
:class:`~monee.model.core.MultiGridBranchModel` and
:class:`~monee.model.core.MultiGridNodeModel`, the multi-grid variants
...
work without it, but registering costs nothing - always decorate.
...
:class:`~monee.model.Network` registers one default grid per carrier
(``mm.EL``, ``mm.WATER``, ``mm.GAS``); pass a custom instance to override.

- :class:`~monee.model.PowerGrid` ``(name, sn_mva=1, vm_pu_max=1.5,
  vm_pu_min=0.5)`` - electrical domain. ``Network.node()`` automatically
  raises a bus's ``vm_pu.min`` to ``grid.vm_pu_min`` when it is at the
  default ``0``: an exact zero lets the :math:`1/v_m` terms of the AC current
